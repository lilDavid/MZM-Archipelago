...
MAKE_ENUM(u8, BeamBombFlags) ENUM_FLAG {
    BBF_NONE        = 0,
    BBF_LONG_BEAM   = 1 << 0,
    BBF_ICE_BEAM    = 1 << 1,
    BBF_WAVE_BEAM   = 1 << 2,
...
MAKE_ENUM(u8, SuitMiscFlags) ENUM_FLAG {
    SMF_NONE         = 0,
    SMF_HIGH_JUMP    = 1 << 0,
    SMF_SPEEDBOOSTER = 1 << 1,
    SMF_SPACE_JUMP   = 1 << 2,
    SMF_SCREW_ATTACK = 1 << 3,
    SMF_VARIA_SUIT   = 1 << 4,
    SMF_GRAVITY_SUIT = 1 << 5,
    SMF_MORPH_BALL   = 1 << 6,
    SMF_POWER_GRIP   = 1 << 7,
};
...
MAKE_ENUM(u8, DiagonalAim) {
    DIAG_AIM_NONE,
    DIAG_AIM_UP,
    DIAG_AIM_DOWN
};

MAKE_ENUM(u8, WeaponHightlighted) ENUM_FLAG {
    WH_NONE          = 0,
    WH_MISSILE       = 1 << 0,
    WH_SUPER_MISSILE = 1 << 1,
    WH_POWER_BOMB    = 1 << 2
};

MAKE_ENUM(u8, SamusPose) {
    SPOSE_RUNNING,
    SPOSE_STANDING,
...
MAKE_ENUM(u8, SamusStandingStatus) {
    STANDING_GROUND,
    STANDING_ENEMY,
    STANDING_MIDAIR,
...
MAKE_ENUM(u8, ArmCannonDirection) {
    ACD_FORWARD,
    ACD_DIAGONALLY_UP,
    ACD_DIAGONALLY_DOWN,
    ACD_UP,
...
MAKE_ENUM(u8, HorizontalMovingDir) {
    HDMOVING_NONE,
    HDMOVING_LEFT,
    HDMOVING_RIGHT
};

MAKE_ENUM(u8, VerticalMovingDir) {
    VDMOVING_NONE,
    VDMOVING_UP,
    VDMOVING_DOWN
};

MAKE_ENUM(u8, EnvEffect) {
    ENV_EFFECT_NONE,
...
MAKE_ENUM(u16, DestructingAction) {
...
MAKE_ENUM(u8, SamusAnimState) {
    SAMUS_ANIM_STATE_NONE,
    SAMUS_ANIM_STATE_SUB_ENDED,
    SAMUS_ANIM_STATE_ENDED
};
...
MAKE_ENUM(u8, ForcedMovement) {
    // Valid for shinesparking poses
    FORCED_MOVEMENT_UPWARDS_SHINESPARK,
    FORCED_MOVEMENT_SIDEWARDS_SHINESPARK,
    FORCED_MOVEMENT_DIAGONAL_SHINESPARK,
    FORCED_MOVEMENT_SHINESPARK_COUNT,

    // Valid for the crawling stopped pose
    FORCED_MOVEMENT_CRAWLING_ARM_CANNON_DOWN = 0,
    FORCED_MOVEMENT_CRAWLING_ARM_CANNON_UP,

    // Valid for morph ball poses
    FORCED_MOVEMENT_BOMB_JUMP = 0,
    FORCED_MOVEMENT_BOMB_JUMP_RIGHT = 10,
    FORCED_MOVEMENT_BOMB_JUMP_UP,
    FORCED_MOVEMENT_BOMB_JUMP_LEFT,
    FORCED_MOVEMENT_MORPH_BALL_BOUNCE_BEFORE_JUMP = 20,
    FORCED_MOVEMENT_BOMB_JUMP_ABOVE = 1 << 7,
    FORCED_MOVEMENT_LAUNCHED_BY_CANNON = 0xF0,
    FORCED_MOVEMENT_MORPH_BALL_BOUNCE_AFTER_FALL = 1,

    // Valid for a lot of poses, used as a transition
    FORCED_MOVEMENT_MID_AIR_FALL = 0,
    FORCED_MOVEMENT_MID_AIR_JUMP,
    FORCED_MOVEMENT_MID_AIR_CARRY,

    // Special cases
    FORCED_MOVEMENT_DAMAGED_BY_RUINS_TEST = UCHAR_MAX
};

MAKE_ENUM(u8, SamusHitboxType) {
    SAMUS_HITBOX_TYPE_STANDING,
    SAMUS_HITBOX_TYPE_CROUCHED,
    SAMUS_HITBOX_TYPE_MORPHED,

    SAMUS_HITBOX_TYPE_NORMAL_COUNT,

    SAMUS_HITBOX_TYPE_ZIPLINE = SAMUS_HITBOX_TYPE_NORMAL_COUNT,

    SAMUS_HITBOX_TYPE_COUNT
};

MAKE_ENUM(u8, SamusBlockHitbox) {
    SAMUS_BLOCK_HITBOX_LEFT,
    SAMUS_BLOCK_HITBOX_RIGHT,
    SAMUS_BLOCK_HITBOX_TOP,

    SAMUS_BLOCK_HITBOX_COUNT,
};

MAKE_ENUM(u8, SamusHitbox) {
    SAMUS_HITBOX_LEFT,
    SAMUS_HITBOX_TOP,
    SAMUS_HITBOX_RIGHT,
    SAMUS_HITBOX_BOTTOM,
...
MAKE_ENUM(u8, SamusCollisionDataFields) {
    SCDF_HITBOX,
    SCDF_BLOCK_HITBOX,
...
MAKE_ENUM(u8, ScrewSpeedFlag) {
    SCREW_SPEED_FLAG_NONE,
    SCREW_SPEED_FLAG_ACTIVE,
